...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('path', '/home/f1/poppin_tires/src/lab5_pure_pursuit-main/logs/waypoints.csv')
        ...
        # Global variable to store the previous waypoint index
    ...
    def odom_callback(self, odom_msg):
        ...
        # Sort indices by distance to car and select 10 closest waypoints (hardcode )
        sorted_indices = np.argsort(self.car_dist) 
        closest_10_indices = sorted_indices[:10]
        
        # Initialize the previous index if it hasn't been set yet
        if self.previous_index is None:
            self.previous_index = closest_10_indices[0]  # Start with the closest waypoint

        # sort the 10 waypoints to how close they are to previous index 
        sorted_by_index_distance = sorted(closest_10_indices, key=lambda idx: abs(idx - self.previous_index))
        closest_index = sorted_by_index_distance[0]
        self.previous_index = closest_index  

        # Use the closest index for the target waypoint
        self.closestPoint = self.waypoints[closest_index]
